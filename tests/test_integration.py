...
class TestFullWorkflow:
    ...
    def test_complete_workflow(self, client):
        ...
        # Edit page
        response = client.post(f'/edit/{page.id}/', {
            'title': 'Workflow Test Page',
            'content': '# Version 2'
        })
        assert response.status_code == 302
        ...
        # Restore old version
        old_revision = PageRevision.objects.filter(page=page, content='# Version 1').first()
        ...
        assert response.status_code == 200
        assert UserActivity.objects.filter(user__username='workflowuser').count() >= 5  # signup, login, create, edit, delete
...
class TestMultiplePages:
    ...
    def test_user_with_multiple_pages(self, client):
        ...
        assert UserActivity.objects.filter(user=user, activity_type='create_page').count() == 3
...
class TestRevisionHistory:
    ...
    def test_extensive_revision_history(self, client):
        ...
        middle_revision = PageRevision.objects.filter(page=page, content='# Version 3').first()
        ...
        latest_revision = PageRevision.objects.filter(page=page, is_current=True).first()
...
"""
        ...
        old_revision = PageRevision.objects.filter(page=page).order_by('created_at').first()
        ...
        edited_revision = PageRevision.objects.filter(page=page, content__contains='**edited**').first()
        ...
        current_revision = PageRevision.objects.filter(page=page, is_current=True).first()
